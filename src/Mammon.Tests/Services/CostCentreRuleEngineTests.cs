...
public class CostCentreRuleEngineTests
{
    ...
    {
		//act+assert
		GetInstance().ClassifyPivot(new CostReportPivotEntry() { PivotName = pivotName, SubscriptionId = subId, ResourceId = resourceId, Cost = new ResourceCost { Cost = 1, Currency = "EUR" } }).Should().Be(expected);
    }
