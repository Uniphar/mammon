...
public class AKSSplitUsageCostActivity(ILogger<AKSSplitUsageCostActivity> logger, IActorProxyFactory actorProxyFactory) : WorkflowActivity<SplitUsageActivityRequest<AKSVMSSUsageResponseItem>, bool>
{
	public override async Task<bool> RunAsync(WorkflowActivityContext context, SplitUsageActivityRequest<AKSVMSSUsageResponseItem> input)
	{
		try
		{
			...
			await actorProxyFactory.CallActorWithNoTimeout<IAKSVMSSActor>(AKSVMSSActor.GetActorId(input.Request.ReportRequest.ReportId, rId.Name, rId.SubscriptionId!), nameof(AKSVMSSActor),
