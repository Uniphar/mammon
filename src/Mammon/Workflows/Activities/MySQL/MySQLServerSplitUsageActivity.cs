...
public class MySQLServerSplitUsageActivity(
	...
	ILogger<MySQLServerSplitUsageActivity> logger,
	IActorProxyFactory actorProxyFactory) : WorkflowActivity<SplittableResourceRequest, bool>
{
	public override async Task<bool> RunAsync(WorkflowActivityContext context, SplittableResourceRequest request)
	{
		try
		{
			...
			await actorProxyFactory.CallActorWithNoTimeout<IMySQLServerActor>(MySQLServerActor.GetActorId(request.ReportRequest.ReportId, rId.Name, rId.SubscriptionId!), nameof(MySQLServerActor),
