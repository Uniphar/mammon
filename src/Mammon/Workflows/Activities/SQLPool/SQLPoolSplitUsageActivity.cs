...
public class SQLPoolSplitUsageActivity
	(ILogger<SQLPoolSplitUsageActivity> logger, IActorProxyFactory actorProxyFactory) : WorkflowActivity<SplitUsageActivityRequest<SQLDatabaseUsageResponseItem>, bool>
{
	public override async Task<bool> RunAsync(WorkflowActivityContext context, SplitUsageActivityRequest<SQLDatabaseUsageResponseItem> input)
	{
		try
		{
			...
			await actorProxyFactory.CallActorWithNoTimeout<ISQLPoolActor>(SQLPoolActor.GetActorId(input.Request.ReportRequest.ReportId, rId.Name, rId.SubscriptionId!), nameof(SQLPoolActor),
