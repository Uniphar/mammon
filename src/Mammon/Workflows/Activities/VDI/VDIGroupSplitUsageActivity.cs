...
public class VDIGroupSplitUsageActivity
	(ILogger<VDIGroupSplitUsageActivity> logger, IActorProxyFactory actorProxyFactory) : WorkflowActivity<SplitUsageActivityGroupRequest<VDIQueryUsageResponseItem>, bool>
{
	public override async Task<bool> RunAsync(WorkflowActivityContext context, SplitUsageActivityGroupRequest<VDIQueryUsageResponseItem> input)
	{
		try
		{
			...
			await actorProxyFactory.CallActorWithNoTimeout<ISplittableVDIPoolActor>(SplittableVDIPoolActor.GetActorId(input.Request.ReportRequest.ReportId, rId.Name, rId.SubscriptionId!), nameof(SplittableVDIPoolActor),
