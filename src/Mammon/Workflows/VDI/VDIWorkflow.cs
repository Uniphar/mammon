...
public class VDIWorkflow : Workflow<SplittableResourceGroupRequest, bool>
{
	...
	{
		(var usageData, bool dataAvailable)= await context.CallActivityAsync<(List<VDIQueryUsageResponseItem> usageData, bool dataAvailable)>(nameof(VDIGroupSplitObtainUsageActivity), request);
