...
public class VDIWorkflow : Workflow<SplittableResourceGroupRequest, bool>
{
	...
	{
		(var usageData, bool dataAvailable)= await context.CallActivityAsync<(IEnumerable<VDIQueryUsageResponseItem> usageData, bool dataAvailable)>(nameof(VDIGroupSplitObtainUsageActivity), request);
		...
		else
		{
			...
			await context.CallChildWorkflowAsync<bool>(nameof(GroupSubWorkflow),
				...
				new ChildWorkflowTaskOptions { InstanceId = $"{nameof(VDIWorkflow)}Group{request.ReportRequest.ReportId}{rId.SubscriptionId}{rId.Name}" });
		}
